...
export function listContentByTeacher(teacherId: string): ContentRow[] {
  return prepare('SELECT * FROM content_items WHERE teacher_id = ? AND deleted_at IS NULL ORDER BY updated_at DESC')
    .all(teacherId) as ContentRow[];
}

export function getContentById(id: string): ContentRow | undefined {
  return prepare('SELECT * FROM content_items WHERE id = ?').get(id) as ContentRow | undefined;
}
...
export function deleteContent(id: string): void {
  prepare("UPDATE content_items SET deleted_at = datetime('now') WHERE id = ?").run(id);
}
